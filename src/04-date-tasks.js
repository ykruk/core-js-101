...
function timeSpanToString(startDate, endDate) {
  ...
  let hour = Math.floor(span / 3600000);
  span -= hour * 3600000;
  let min = Math.floor(span / 60000);
  span -= min * 60000;
  let sec = Math.floor(span / 1000);
  span -= sec * 1000;
  let msec = span;
...
function angleBetweenClockHands(date) {
  ...
  const min = dateValue.getUTCMinutes();
  let hour = dateValue.getUTCHours();
  hour = hour > 12 ? hour -= 12 : hour;
  hour = hour * 30 + min / 2;
  let angle = Math.abs(hour - min * 6);
