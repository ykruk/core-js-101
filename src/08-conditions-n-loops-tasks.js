...
function getFizzBuzz(num) {
  return (num % 3 && num % 5 && num) || (!(num % 3) && !(num % 5) && 'FizzBuzz') || (!(num % 3) && 'Fizz') || (!(num % 5) && 'Buzz');
}
...
function findFirstSingleChar(str) {
  const count = str.split('').reduce((acc, el) => {
    if (Object.keys(acc).includes(el)) {
      acc[el] += 1;
    } else {
      acc[el] = 1;
    }
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const leftNum = a < b ? a : b;
  const rightNum = b > a ? b : a;
  return `${leftBracket}${leftNum}, ${rightNum}${rightBracket}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  const sum = number.split('').reverse().reduce((acc, el, i) => {
    let add = 0;
    if (i % 2 === 1) {
      if (el * 2 > 9) {
        add = Math.floor((el * 2) / 10) + ((el * 2) % 10);
      } else {
        add = el * 2;
      }
    } else {
...
function getDigitalRoot(num) {
  if (num <= 9) return num;
  const res = num.toString().split('').reduce((acc, el) => acc + +el, 0);
...
function isBracketsBalanced(str) {
  ...
  const testStorage = str.split('').reduce((acc, el) => {
    if (brackets[acc[acc.length - 1]] === el) {
      acc.pop();
    } else {
      acc.push(el);
    }
  ...
  if (testStorage.length === 0) return true;
  return false;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < checkPath.length; i += 1) {
    const el = checkPath[i];
    for (let j = 1; j < pathes.length; j += 1) {
      const element = pathes[j];
      if (element[i] !== el) {
        check = false;
        break;
      }
  ...
  for (let i = 0; i < m1.length; i += 1) {
    ...
    for (let j = 0; j < m2[0].length; j += 1) {
      ...
      for (let y = 0; y < m2.length; y += 1) {
        matrixProduct[i][j] += m1[i][y] * m2[y][j];
      }
    }
  }
...
function evaluateTicTacToePosition(position) {
  const winPositions = [position[0][0] + position[0][1] + position[0][2],
  ...
  if (winPositions.indexOf('XXX') >= 0) {
    return 'X';
  }
  if (winPositions.indexOf('000') >= 0) {
    return '0';
  }
  return undefined;
}
